...
1. **Frontend Interaction:** A user enters their name and email into an HTML form on the web page.
2. **Form Submission:** When the user clicks "Add User," the data is sent to the Flask backend through a POST request.
3. **Database Storage:** The backend receives this information and executes a SQL command to insert the data into the users table in `database.db`.
4. **Data Retrieval:** After saving the new user, the application fetches all current records from the SQLite database.
5. **Dynamic Display:** The retrieved data is sent back to the frontend, where it is displayed in a styled table for the user to view.
